                        ...
                        while self.running:
                            ...
                            for ch_mon in channels:
                                try:
                                    ...
                                    imon_l = device.get_ch_param(0, [ch_mon], 'IMonL')[0]
                                    imon_h = device.get_ch_param(0, [ch_mon], 'IMonH')[0]
                                    results.append((0, ch_mon, vmon, imon_l, imon_h))
                                except hv.Error:
                                    results.append((0, ch_mon, 0.0, 0.0, 0.0))
...
class MonitoringApp(QMainWindow):
    ...
    def setup_ui(self):
        ...
        curr_h_header = QLabel("Current H (uA):"); curr_h_header.setFont(font_large); curr_h_header.setStyleSheet(style_current)
        curr_l_header = QLabel("Current L (uA):"); curr_l_header.setFont(font_large); curr_l_header.setStyleSheet(style_current)
        status_layout.addWidget(volt_header, 4, 0)
        status_layout.addWidget(curr_h_header, 5, 0)
        status_layout.addWidget(curr_l_header, 6, 0)
        
        self.hv_v_labels, self.hv_ih_labels, self.hv_il_labels = {}, {}, {}
        for ch in self.hv_channels:
            v_label = QLabel(f"Ch{ch}: -"); v_label.setFont(font_large); v_label.setStyleSheet(style_voltage)
            ih_label = QLabel(f"Ch{ch}: -"); ih_label.setFont(font_large); ih_label.setStyleSheet(style_current)
            il_label = QLabel(f"Ch{ch}: -"); il_label.setFont(font_large); il_label.setStyleSheet(style_current)
            status_layout.addWidget(v_label, 4, ch + 1)
            status_layout.addWidget(ih_label, 5, ch + 1)
            status_layout.addWidget(il_label, 6, ch + 1)
            self.hv_v_labels[ch] = v_label
            self.hv_ih_labels[ch] = ih_label
            self.hv_il_labels[ch] = il_label
    ...
    def setup_logger(self):
        ...
        for ch in self.hv_channels: header.extend([f'Ch{ch}_V', f'Ch{ch}_I_L', f'Ch{ch}_I_H'])
    ...
    def update_indicators(self):
        ...
        for ch in self.hv_channels:
            data = self.latest_data['hv'].get(ch, {'v': 0, 'il': 0, 'ih': 0})
            self.hv_v_labels[ch].setText(f"Ch{ch}: {data['v']:.2f}")
            self.hv_ih_labels[ch].setText(f"Ch{ch}: {data['ih']:.4f}")
            self.hv_il_labels[ch].setText(f"Ch{ch}: {data['il']:.4f}")
    ...
    def update_graphs(self):
        ...
        for ch in self.hv_channels:
            hv = self.latest_data['hv'].get(ch, {'v': 0, 'ih': 0})
            self.graph_volt_data[ch].append(hv['v']); self.graph_curr_data[ch].append(hv['ih'])
        ...
        try:
            with open(self.log_file_path, 'a', newline='') as f:
                ...
                for item in buffer_copy:
                    ...
                    for ch in self.hv_channels:
                        hv = item['hv'].get(ch, {'v': 0, 'il': 0, 'ih': 0}); row.extend([hv['v'], hv['il'], hv['ih']])
                    writer.writerow(row)
    ...
    def update_caenhv_data(self, results):
        for _, ch, vmon, imon_l, imon_h in results:
            self.latest_data['hv'][ch] = {'v': vmon, 'il': imon_l, 'ih': imon_h}
